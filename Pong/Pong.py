...
#paddle mover
def MovePaddle1(body, shape, left, right):
    deltaX = 0
    pos = body.position
    if left:
...
#Second Paddle Mover
def MovePaddle2(body, shape, left, right):
    deltaX = 0
    pos = body.position
    if left:
        deltaX = deltaX - PADDLEMOVE
...
ballBody.position = (random.randint(25,775), 35)
...
while not done:
    ...
    MovePaddle1(paddleBody1, paddleShape,leftArrowDown,rightArrowDown)
    MovePaddle2(paddleBody2, paddleShape, AKeyDown, DKeyDown)
    space.step(1/60.0)
    screen.fill((0, 0, 0))
    ...
    textX = int(200 - (textSize[0] / 2))
    screen.blit(scoreSurface, (textX, 300))

    scoreSurface2 = font.render(str(score2), True, (255, 255, 255))
    textSize = scoreSurface2.get_size()
    textX = int(600 - (textSize[0] / 2))
    screen.blit(scoreSurface2, (textX, 300))
